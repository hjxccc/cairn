...
AI agent 干活有三个每天都撞的问题，cairn 各给一个"零机器"的答案：

**① 失忆——每开新会话，agent 就忘光了。** 项目背景、规范、上次做到哪，全得重讲一遍。
→ cairn 把这些**沉在 repo 里**（INDEX / progress / pitfalls / SOP 都是文件）：会话会死、仓库不忘，新会话一条 `grep` 就捞回来。

**② 续不上——长任务跨好几个会话，"上次卡在哪"没处放。** 写进独立指针文件（`.current-task` 一类）两周就过期，烂成误导。
→ cairn 把状态**长在任务自己身上**（`progress.md` + INDEX 的 🚧）：`grep "^- 🚧"` 一眼看全部在途，零维护。

**③ 重复踩坑——同一个坑，你 / 同事 / 另一个 agent 又从头踩一遍。**
→ cairn 让**坑账一行一坑 + SOP 一等公民**：动手前先 grep 历史，命中就复用，不重新探索。

### 对比传统两种做法（都不理想）

| | 每次手动交代 | 重型自动注入框架 | **cairn** |
|---|---|---|---|
| 跨会话记忆 | ❌ 全靠你重讲 | ✅ 但塞满上下文 | ✅ 痕迹在 repo，按需捞 |
| 会话启动开销 | 轻 | ❌ 每次注入 ~22KB | ✅ ~260 token 指针 |
| 维护成本 | 0（但啥也没沉淀） | ❌ 要喂养的机器会烂 | ✅ 收尾垒一行，零机器 |
| 找历史 | 翻聊天记录 | 全量预注入，要的那条淹在里面 | ✅ 一条 grep 命中 |
| 迁移 / 共享 | — | 绑死工具 | ✅ 纯 markdown，拷走即用 |

> 不是说"自动注入"错——它是正当设计。只是在 **5 个月 / 148 个任务**的实战里，它被证明**喂不动、还撑爆上下文**（下方"尸检"表是原始判决）。cairn 是对同一批痛点的相反下注：**平时不喂（启动极轻），要用时一条 grep 捞。**
