...
AI agent 干活有三个每天都撞的问题，cairn 各用一套"无运行时"的方式接住：

**① 项目知识困在会话/工具里——每开新会话，agent 就得重来。** 背景、规范、上次的结论，都在上一段对话里，新会话看不到。
→ cairn 把这些**沉在 repo 里**（INDEX / summary / pitfalls / SOP 都是文件）：会话会死、仓库不忘，新会话 `grep` 一下就能捞到那条（词面命中是快路径，没中就换个词或直接读那份小索引）。

**② 进行中任务缺一个能冷启动的恢复点——长任务跨好几个会话，"上次卡在哪"没处放。** 写进独立指针文件（`.current-task` 一类）两周就过期，烂成误导。
→ cairn 把状态**长在任务自己身上**（`progress.md` + INDEX 的 🚧）：`grep "^- 🚧"` 一眼看全部在途；状态随收尾动作更新，漂移由只读的 `doctor.sh` 兜底——不引入独立状态机。

**③ 同一个坑反复踩——你 / 同事 / 另一个 agent 又从头踩一遍。**
→ cairn 让**坑账一行一坑 + SOP 一等公民**：动手前先 grep 历史，命中就复用，不重新探索。

### 三种路线的取舍（不是优劣，是不同的成本模型）

| | 每次手动交代 | 自动注入框架 | **cairn** |
|---|---|---|---|
| 跨会话记忆 | ❌ 全靠你重讲 | ✅ 预先注入，但占上下文 | ✅ 痕迹在 repo，检索时才取 |
| 会话启动开销 | 轻 | 每次注入一份规范（某项目实测 ~22KB） | 约 1.5KB 规则片段 + 轻量指针 |
| 状态维护 | 0（但啥也没沉淀） | 框架维护生命周期状态 | 无独立状态机；随收尾更新，doctor 查漂移 |
| 找历史 | 翻聊天记录 | 全量预注入，要的那条淹在里面 | grep 命中（快路径）→ 没中读小索引 |
| 平台耦合 | — | 需要框架协议 + 适配层 | 核心是 markdown，不依赖平台；团队层随 clone 走、个人层默认 gitignore |

> 自动注入不是错——它适合规范稳定、流程强约束的团队，赌的是"预先全喂"。cairn 是另一套成本模型的相反下注：**平时不为每次会话付费，只在检索时付费**（这个取舍的完整推理见 [决策 004](docs/decisions/004-grep-over-vector-search.md)）。下方"尸检"表，是作者在一个多仓库项目上跑满 5 个月 / 148 个任务后的原始判决。
