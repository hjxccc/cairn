...
重型框架的自动注入适合"想零成本加载 + 流程强约束"的团队；单个规则文件适合"极简单文件"的项目；cairn 适合工作是即兴的（排障、补数、救火）、又想要持久可 grep、可团队共享、还没有机器要喂养——它的赌注是：**在检索时付费，不在每次会话付费**（[实测 token 成本对比 →](docs/token-cost.zh-CN.md)）。
