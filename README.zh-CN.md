...
> **每次会话的账（实测）**：cairn 常驻 **~350 tok**（5 行前言 + 你的在途面板）；重型框架每次注入 **~5000–7000 tok** 静态规范——**差 ~15–20×**。cairn 的 SKILL 只在你真做 cairn 动作时载一次（~3–4.3k），干完就不再收费。量你自己的：`./.cairn/scripts/cairn-cost.sh`。（[完整实测与方法 →](docs/token-cost.zh-CN.md)）
...
从"实测活下来的做法"，到由此定下的原则，再到外部依据——一条线：
...
- **agent 任务追踪器**（Beads）：面向多 agent 并行执行的依赖图数据库；cairn 记的是"什么/为什么/怎么做"的人类可读留痕，纯文件。
