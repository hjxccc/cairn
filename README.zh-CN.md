...
→ cairn 把这些**写进仓库**（`INDEX.md`、任务总结、坑账、SOP 都是文件）：会话会关，文件不丢，新会话 `grep` 一下就把那条捞回来。

**② 活干到一半，换个会话就"我上次弄到哪了？"** 长任务跨好几天、好几次对话，进度没个准地方放（记在会过期的指针文件里，两周后就没人信了）。
→ cairn 把进度**记在任务自己的 `progress.md`**、INDEX 行首标个 🚧：一条 `grep "^- 🚧"` 看到所有没干完的活，接着上次的"下一步"往下走，漂移了有只读的 `doctor.sh` 兜底。
...
不是每项都赢，是不同的成本模型。诚实拆一下：

| 维度 | 手动规则文件（`.cursorrules`/`CLAUDE.md`） | 自动注入框架 | **cairn** |
|---|---|---|---|
| 跨会话记忆 | 无——每次重讲 | 预先注入（有，但占上下文） | 痕迹在 repo，检索时才取 |
| 会话启动开销 | 轻 | 每次注入一份规范（某项目实测 ~22KB） | ~1.5KB 规则 + 轻量指针 |
| 进度 / 状态 | 没处放 | 框架维护生命周期状态（可能过期） | 长在任务 `progress.md` + INDEX 🚧 |
...
| **cairn 的代价（诚实）** | — | — | **要你主动 grep；词面检索会漏（靠关键词标签 + 读小索引兜底）；约定要人去遵守（收尾垒一行）** |

自动注入适合"想零成本加载 + 流程强约束"的团队；单个规则文件适合"极简单文件"的项目；cairn 适合工作是即兴的（排障、补数、救火）、又想要持久可 grep、可团队共享、还没有机器要喂养——它的赌注是：**在检索时付费，不在每次会话付费。**
